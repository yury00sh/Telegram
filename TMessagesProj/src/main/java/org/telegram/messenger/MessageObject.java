...
public class MessageObject {
    ...
    public MessageObject(int accountNum, TLRPC.TL_channelAdminLogEvent event, ArrayList<MessageObject> messageObjects, HashMap<String, ArrayList<MessageObject>> messagesByDays, TLRPC.Chat chat, int[] mid, boolean addToEnd) {
        ...
        if (event.action instanceof TLRPC.TL_channelAdminLogEventActionChangeTitle) {
            ...
            if (action.new_value) {
                messageText = replaceWithLink(LocaleController.getString("EventLogNoForwardsToggledOn", R.string.EventLogNoForwardsToggledOn), "un1", fromUser);
            } else {
                messageText = replaceWithLink(LocaleController.getString("EventLogNoForwardsToggledOff", R.string.EventLogNoForwardsToggledOff), "un1", fromUser);
            }
        } else {
